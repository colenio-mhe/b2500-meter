...
The emulator rounds all power values to one decimal place (matching a real Shelly Pro 3EM) and always serializes them as real float literals (e.g., `200.0` instead of `200`), which the battery firmware requires. It uses a **Synchronous Blocking Strategy**: by waiting for a fresh measurement before responding to the battery's request, it synchronizes the battery's internal PI-regulator with the actual update rate of your power meter. This eliminates "sawtooth" patterns and minimizes grid leakage.